...
class LeaveApplicationController extends Controller
{
    ...
    public function create()
    {
        ...
        $leaveApps = LeaveApplication::orderBy('date_from', 'ASC')->whereDate('created_at', '>', '2021-12-30')->get();
    ...
    public function approve(LeaveApplication $leaveApplication)
    {
        ...
        //$leaveApplication->user->notify(new StatusUpdate($leaveApplication));
        NotifyUserEmail::dispatch($leaveApplication)->delay(now()->addMinutes(1));
        NotifyAuthorityEmail::dispatch($leaveApplication, $this->leaveService)->delay(now()->addMinutes(1));
    ...
    public function cancel(LeaveApplication $leaveApplication, Request $request)
    {
        ...
        if ($leaveApplication->status == "APPROVED") {
            ...
            if ($leaveApplication->leave_type_id == "12") {
                ...
                if ($leaveApplication->remarks == "Apply") {
                    $apply_claim = ReplacementRelation::where('leave_id', $leaveApplication->id)->first();
                    $claim_leave_app = LeaveApplication::where('id', $apply_claim->claim_id)->first();

                    //Get replacement leave balance
                    $balanceLeave = LeaveBalance::where('user_id', $leaveApplication->user_id)->where('leave_type_id', 12)->first();
                    //Get replacement leave taken
                    $takenLeave = TakenLeave::where('user_id', $leaveApplication->user_id)->where('leave_type_id', 12)->first();
                    ...
                    if ($claim_leave_app->status == 'TAKEN') {
                        $claim_leave_app->status = 'APPROVED';
                        $claim_leave_app->save();
                    }
                } else if ($leaveApplication->remarks == "Claim") {
